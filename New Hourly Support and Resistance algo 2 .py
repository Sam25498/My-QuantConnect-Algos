...
class SwimmingFluorescentPinkShark(QCAlgorithm):
    ...
    def OnData(self, data):
        ...
        for symbol, symbolData in self.Data.items(): #Return the dictionary's key-value pairs:
            ...
            nextSupportLevel = max([num for num in supports if num < current_price])#supports[0]
            
            #Getting the next support level
            nextResistanceLevel = min([nom for nom in resistances if nom > current_price])#resistances[0]

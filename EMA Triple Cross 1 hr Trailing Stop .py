...
condTrailingStop = None
condTrailingStopS = None
        ...
        for symbol, symbolData in self.Data.items(): #Return the dictionary's key-value pairs:
            ...
            if self.Portfolio[symbol].Invested:
                if self.isLong:
                        ...
                        if (profit / cash) > self.trailingSL:
                                    self.stopLossLevel = self.stopLossLevel + self.trailingSL
                                    condTrailingStopS = (profit / cash) < self.stopLossLevel
                                    self.trailingSL += 0.05
                       ...
                       # if condTrailingStopS:
                                    #self.Liquidate(symbol)
                                    #self.Log(f"{self.Time} Short Position Trailing Stop Loss at {current_price}") 
